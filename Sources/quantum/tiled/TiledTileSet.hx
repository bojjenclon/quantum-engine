...
class TiledTileSet
{
	...
	public function new(data : FlxTiledTileAsset, rootPath : String = "")
	{
		...
		{
			var sourcePath = Path.normalize(rootPath + source.att.source).replace(' ', '_').replace('.', '_');
			var blob = Assets.blobs.get(sourcePath);
			if (blob != null)
			{
				source = new Access(Xml.parse(blob.toString()));
...
typedef FlxTiledTileAsset = Access;
